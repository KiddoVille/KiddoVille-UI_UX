<?php
    ...
    $packageData = isset($data['Package']) ? [$data['Package']] : [];
?>
...
<body>
    <form method="post" id="details" enctype="multipart/form-data" action="<?=ROOT?>/Child/ChildPackage/handleFormSubmission">
        ...
        <div class="Profilecard">
            ...
            <div class="ProfileContainer" style="flex-direction: column !important;">
                ...
                <div class="datacon" style="margin-top: 500px; position: fixed; flex-direction: row;">
                <div class="data" style="margin-top: 40px;">
                <button type="button" id="add-btn" 
                    style="font-weight: 600; width: 200px; font-size: 20px; padding: 7px; background-color:rgb(67, 99, 154) !important; color: white !important; margin-bottom: -40px;"
                    onclick="window.location.href='<?= ROOT . '/' . $location ?>'">
                    <i class="fa fa-chevron-left" style="margin-left: 4px;"></i> Back
                </button>
                </div>
                    <div class="data" style="margin-top: 40px; margin-left: 270px; ">
                        <button type="submit" id="submit" style="font-weight: 600; width: 200px; font-size: 20px; padding: 7px; background-color:rgb(67, 99, 154) !important; color: white !important; margin-bottom: -40px;"> Save <i class="fa fa-chevron-right" style="margin-left: 4px;"></i></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function fetchPackages(data) {
            ...
            // function selecteddates(Current) {
            //     console.log("The Current package = ", Current);
            ...
            //     days.forEach(day => {
            //         let shortDay = day.substring(0, 3); // Convert "Sunday" -> "Sun"
            //         let checkbox = document.getElementById(`day-${shortDay}`);
            //         console.log(Current[day])
            //         if (checkbox && Current[day] == 1) {
            ...
            function selecteddates(Current) {
                ...
                // Map of full day names to their corresponding property names in Current object
                const dayMapping = {
                    'Sunday': 'Sunday',
                    'Monday': 'Monday',
                    'Tuesday': 'Tuesday',
                ...
                days.forEach(day => {
                    ...
                    if (checkbox) {
                        // Check if the day is enabled (set to 1) in the Current object
                        const isDayEnabled = Current[0][dayMapping[day]] == 1;
                        
                        // Set checkbox state based on the Current object
                        checkbox.checked = isDayEnabled;
                        
                        // If you need the attribute for styling purposes
                        if (isDayEnabled) {
                            checkbox.setAttribute("checked", "checked");
                            checkbox.checked = true;
                            checkbox.setAttribute("checked", "checked");
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let Current = <?= json_encode($packageData) ?>;
            console.log("Initial Package Data:", Current);
            updatePackagesDisplay(Current);
            selecteddates(Current);

            document.querySelectorAll("input[name='selectedDays[]']:checked").forEach(input => {
                let shortDay = input.value.substring(0, 3); // Convert "Sunday" -> "Sun"
                let dayDiv = document.querySelector(`.day[data-day="${shortDay}"]`);
                if (dayDiv) {
            ...
            document.querySelectorAll("input[name='selectedDays[]']:checked").forEach(input => {
                let dayDiv = document.querySelector(`.day[data-day="${input.value}"]`);
                if (dayDiv) {
                    dayDiv.classList.add("selectday");
                }
            });
